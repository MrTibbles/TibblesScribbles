define(['jquery', 'backbone', 'app_config'], function($, Backbone, app_config) {
  var exterior_view = Backbone.View.extend({
    ...
    initialize: function() {
      var model_bucket = app_config.name;
      if(model_bucket === 'priusplus'){
        model_bucket = 'prius-plus';
      }else if(model_bucket === 'priusplugin'){
        model_bucket = 'prius-plug-in';
      }
      
      this.spin_location = '//s3-eu-west-1.amazonaws.com/shareduat.toyotaretail.co.uk/widgets/configure/images/spins/' + model_bucket + '/';
