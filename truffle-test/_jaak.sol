...
contract Jaak is mortal {
  ...
  /// @notice incrementJaakCount is used to keep of track of total number of times a track has been shared within th network.
  /// @notice would we want to keep track of WHO has shared th track? - user profiling etc
  /// @param id trackID (swarm hash of jaak manifest)
  function incrementJaakCount(bytes32 id) {
    Track track = tracks[id];
    track.jaakCount++;
  }
